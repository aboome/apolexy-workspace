package com.yh.apoplexy.patient.open.common.filter;
...
public class WebFilter extends BaseFilter {
    ...
    public boolean requestFilter(HttpServletRequest request,
     ...
     * @return
     */
    public boolean illegalCharactersValidate(String currentURL,
     ...
     * @return
     */
    private boolean hasUrlPermission(HttpServletRequest request,
                                     String currentURL) {
        // 判断该用户是否有该URL的访问权限
        List<String> userUrlAuthorizeInfo = (List<String>) SessionUtil
                .getSessionValue(request, SessionKey.AUTHROIZE_INFO_URL);
    ...
    private boolean isWhiteUrl(String currentURL) {
        ...
        if ("/login".equals(currentURL) || "/logout".equals(currentURL)
                || "/error".equals(currentURL) || "/error1".equals(currentURL)
                || "/app/open_api".equals(currentURL) || "/gateway/open_api".equals(currentURL)) {
            return true;
        }
        return false;
    }

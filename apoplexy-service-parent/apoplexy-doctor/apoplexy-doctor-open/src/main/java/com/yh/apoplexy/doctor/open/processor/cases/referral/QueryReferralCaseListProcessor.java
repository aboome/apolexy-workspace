...
import com.yh.apoplexy.doctor.open.request.cases.referral.QueryReferralCaseListRequest;
import com.yh.apoplexy.doctor.open.response.base.DoctorAppBaseResponse;
import com.yh.apoplexy.doctor.open.response.cases.referral.QueryReferralCaseListResponse;
...
/**
 * 查询病例转诊列表(doc-0019)处理类
 * Created by wunder on 16/9/8 09:56.
 */
public class QueryReferralCaseListProcessor extends DoctorAppBaseServiceProcessor {

    private static final Logger LOGGER = LoggerFactory.getLogger(QueryReferralCaseListProcessor.class);
    ...
    protected DoctorAppBaseResponse doProcess(DoctorAppBaseRequest requestObject) {
        ...
        QueryReferralCaseListResponse queryReferralCaseListResponse = new QueryReferralCaseListResponse();
        ...
        QueryReferralCaseListRequest queryReferralCaseListRequest = JSONObject.parseObject(requestObject.getParameter().toString(), QueryReferralCaseListRequest.class);

        if (null == queryReferralCaseListRequest) {
        ...
        Result validateResult = CommonValidator.validate(queryReferralCaseListRequest);
        ...
        Long userId = Long.parseLong(queryReferralCaseListRequest.getUserId());

        String type = queryReferralCaseListRequest.getType();

        String status = queryReferralCaseListRequest.getStatus();

        int currPage = Integer.parseInt(queryReferralCaseListRequest.getCurPage());

        int pageSize = Integer.parseInt(queryReferralCaseListRequest.getPageSize());
        ...
        if (Constants.ReferralCaseStatus.ALL.equals(status)){
        ...
        if (Constants.Range.ONE_BY_ONE_BY_ONE.equals(type)) {
            queryReferralCaseDto.setDoctorRange(Constants.Range.ONE_BY_ONE_BY_ONE);
        }else if (Constants.Range.ALL.equals(type)){
            queryReferralCaseDto.setCaseRange(Constants.Range.ALL);
        }else {
            response.setResultcode(APPResponseCodeConstants.FAILED_PARAMETER_ERROR);
            return response;
        }

        List<ReferralCaseInfoDto> referralCaseInfoDtoList = referralCaseService.selectListByPage(queryReferralCaseDto, page);
        ...
        if (CollectionUtils.isEmpty(referralCaseInfoDtoList)) {
            ...
            queryReferralCaseListResponse.setList(referralCaseItemList);
            queryReferralCaseListResponse.setCurPage(String.valueOf(page.getCurrentPage()));
            queryReferralCaseListResponse.setPageSize(String.valueOf(page.getPageSize()));
            queryReferralCaseListResponse.setTotalCount(String.valueOf(page.getCount()));
        ...
        response.setParameter(queryReferralCaseListResponse);

...
import com.yh.apoplexy.doctor.open.request.cases.ast.QueryAstCaseListRequest;
import com.yh.apoplexy.doctor.open.response.base.DoctorAppBaseResponse;
import com.yh.apoplexy.doctor.open.response.cases.ast.QueryAstCaseListResponse;
...
/**
 * 查询AST病例列表(doc-0034)处理类
 * Created by wunder on 16/9/11 10:57.
 */
public class QueryAstCaseListProcessor extends DoctorAppBaseServiceProcessor {

    private static final Logger LOGGER = LoggerFactory.getLogger(QueryAstCaseListProcessor.class);
    ...
    protected DoctorAppBaseResponse doProcess(DoctorAppBaseRequest requestObject) {
        ...
        QueryAstCaseListResponse queryAstCaseListResponse = new QueryAstCaseListResponse();
        ...
        QueryAstCaseListRequest queryAstCaseListRequest = JSONObject.parseObject(requestObject.getParameter().toString(),QueryAstCaseListRequest.class);

        if (null == queryAstCaseListRequest) {
        ...
        Result validateResult = CommonValidator.validate(queryAstCaseListRequest);
        ...
        Long userId = Long.parseLong(queryAstCaseListRequest.getUserId());

        int currPage = Integer.parseInt(queryAstCaseListRequest.getCurPage());

        int pageSize = Integer.parseInt(queryAstCaseListRequest.getPageSize());
        ...
        List<AstCaseInfoDto> astCaseInfoDtoList = astCaseService.selectListByPage(queryAstCaseDto,page);
        ...
        if (CollectionUtils.isEmpty(astCaseInfoDtoList)){
            ...
            queryAstCaseListResponse.setList(astCaseItemList);
            queryAstCaseListResponse.setCurPage(String.valueOf(page.getCurrentPage()));
            queryAstCaseListResponse.setPageSize(String.valueOf(page.getPageSize()));
            queryAstCaseListResponse.setTotalCount(String.valueOf(page.getCount()));

            response.setParameter(queryAstCaseListResponse);
            return response;

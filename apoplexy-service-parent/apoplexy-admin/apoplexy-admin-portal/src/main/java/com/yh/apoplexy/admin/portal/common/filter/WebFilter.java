package com.yh.apoplexy.admin.portal.common.filter;

import com.yh.apoplexy.common.constants.CommonConst;
import com.yh.apoplexy.common.constants.SessionKey;
import com.yh.apoplexy.common.constants.SystemResultCode;
import com.yh.apoplexy.common.dto.UserSession;
import com.yh.apoplexy.common.utils.RegexUtil;
...
public class WebFilter extends BaseFilter {
    ...
    public boolean requestFilter(HttpServletRequest request, HttpServletResponse response, String currentURL) throws IOException, ServletException {
...
//		}
     ...
     * @throws IOException
     * @throws ServletException [参数说明]
     * @throws throws           [违例类型] [违例说明]
     * @see [类、类#方法、类#成员]
     */
    public static boolean filterFailed(HttpServletRequest request, HttpServletResponse response, String filterFailedCode) throws IOException, ServletException {
        ...
        if (WebUtil.isAjaxRequest(request)) {
            ...
            if (CommonConst.REQUEST_METHOD_GET.equals(requestMethod)
                    && filterFailedCode
                    .startsWith(SystemResultCode.GLOBAL_ERROR_USER_SESSION_OUT_TIME)) {
                String dispatcherUrl = request.getContextPath() + "/login";
                response.sendRedirect(dispatcherUrl);
            } else {
                String dispatcherUrl = null;
                if (SystemResultCode.GLOBAL_ERROR_ILLEGAL_CHARACTERS
                        .equals(filterFailedCode)
                        || SystemResultCode.GLOBAL_ERROR_NO_PERMISSION
                        .equals(filterFailedCode)) {
                ...
                ((HttpServletRequest) request).getRequestDispatcher(
                        dispatcherUrl).forward(request, response);
            }
        }
        return false;
    }
        ...
        for (Map.Entry<String, String[]> entry : allParameter.entrySet()) {
            ...
            for (String val : values) {
                ...
                if (RegexUtil.isIllegalCharacters(val)) {
                    securityLogger.info("Illegal character access,string={}",
                            val);
                    return false;
                }
            }
    ...
    private boolean hasUrlPermission(HttpServletRequest request, String currentURL) {
        // 判断该用户是否有该URL的访问权限
        List<String> userUrlAuthorizeInfo = (List<String>) SessionUtil.getSessionValue(request, SessionKey.AUTHROIZE_INFO_URL);
    ...
    private boolean isWhiteUrl(String currentURL) {
        ...
        if ("/login".equals(currentURL) || "/logout".equals(currentURL)
                || "/404".equals(currentURL) || "/500".equals(currentURL)
                || "/loginSubmit".equals(currentURL)
                || currentURL.contains("/image/view")) {
            return true;
        }
        return false;
    }
